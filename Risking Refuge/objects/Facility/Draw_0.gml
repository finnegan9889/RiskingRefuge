...
if(state == 1){  //show resource info
	...
	if(!instance_exists(confirmButton)){
		instance_create_depth(self.x - 38, self.y - 36, -1, confirmButton);
		instance_create_depth(self.x - 38, self.y + 4, -1, confirmButton);
		instance_create_depth(self.x - 38, self.y + 44, -1, confirmButton);
		instance_create_depth(self.x - 38, self.y + 84, -1, confirmButton);
	}
	
}else{ //hide info
	if(instance_exists(confirmButton)){
		instance_deactivate_object(confirmButton);	
	}
}